...
class OCMSMAYAElementTree:
    ...
    def __list_elements(self, mode=1):
        ...
        if mode > 1 or mode < 0:
            ...
            if mode == 0:
                elems = cmds.listRelatives(
                    self.root, allDescendents=True, path=True, type="transform"
                )
                _root = self.root
            elif mode == 1:
                elems = cmds.listRelatives(
                    self.root, allDescendents=True, fullPath=True, type="transform"
                )
                _root = "|" + self.root
            elems.reverse()
    ...
    def __list_tags(self, element):
        ...
        if bool(attr):
            for at in attr:
                if (
                    not cmds.attributeQuery(at, node=element, numberOfChildren=True)
                    == None
                ):
                    tags.append(at)
        else:
...
class OCMSMAYAElement(OCMSMAYAElementTree):
    ...
    def __element_parent(self, element):
        node = cmds.listRelatives(
            element, allDescendents=True, fullPath=True, parent=True, type="transform"
        )
        if bool(node):
            _parent = node[0]
        else:
    ...
    def __get_transform(self, element):
        ...
        if not _t[0] == 0 or not _t[1] == 0 or not _t[2] == 0:
            __Transform["Transform"]["position"] = {
                "x": str(_t[0]),
                "y": str(_t[1]),
                "z": str(_t[2]),
            }
        ...
        if not _r[0] == 0 or not _r[1] == 0 or not _r[2] == 0:
            __Transform["Transform"]["rotation"] = {
                "x": str(_r[0]),
                "y": str(_r[1]),
                "z": str(_r[2]),
            }
...
class OCMSMAYAETWrite:
    ...
    def writxml(self, mode, _file):
        if os.path.exists(_file) or True:
...
def op_browser_explorer(self):
    _file_path = self._browser_path
...
def write_xml(self):
    ...
    from oe.module.importres import operator as op
