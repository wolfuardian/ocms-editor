...
class XML(core.XML):
    ...
    def iterator(cls, root, tag=None, attr=None, kwd=None):
        if attr or kwd is not None:
            if attr is None:
                tool.Log.warning(__name__, "attr should not be empty when kwd used.")
            if kwd is None:
                tool.Log.warning(__name__, "kwd should not be empty when attr used.")
    ...
    def enumerator(cls, elems, attr, mode=0, out_attr=None, test_print=False):
        ...
        if test_print:
            return tool.String.list_to_string(lst)
