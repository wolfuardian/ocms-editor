...
class PathStore:
    ...
    def __validate(self):
        if not self.__path or self.__path == "":
            ...
            self.__is_exists = tool.IO.is_exists(self.__path)
            self.__is_dir = tool.IO.is_dir(self.__path)
            self.__is_file = tool.IO.is_file(self.__path)
            self.__is_xml = tool.IO.is_xml(self.__path)
...
class RegistryStore:
    ...
    def _fetch(self):
        self.__value = tool.Registry.get_value(self.__key, self.__sub, self.__name, "")

    def set(self, value):
        self.__value = value
        tool.Registry.set_value(self.__key, self.__sub, self.__name, self.__value)
        return self.__value
...
class QtGroupVBoxStore:
    ...
    def add_widget(self, parent_id, widget_id, widget):
        ...
        print(f"parent_id: {parent_id}, id: {widget_id}")
