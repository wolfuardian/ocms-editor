...
class ParseXMLData:
    ...
    def get_xmlstring(cls):
        return tool.IO.read_utf16(cls.path)

    @classmethod
    def get_root(cls):
        return tool.XML.root(cls.xmlstring)

    @classmethod
    def get_nodes_objects(cls):
        return tool.XML.iterator(cls.root, tag="Object")

    @classmethod
    def get_nodes_datasource(cls):
        return tool.XML.iterator(cls.root, tag="DataSource")

    @classmethod
    def get_data_objects(cls):
        return tool.XML.enumerator(cls.nodes_objects, attr="name", mode=1)

    @classmethod
    def get_data_datasource(cls):
        return tool.XML.enumerator(cls.nodes_datasource, attr="ProductType")[0]

    @classmethod
    def get_tags(cls):
        return tool.XML.collect_attrs(cls.root, "tag")

    @classmethod
    def get_attrs(cls):
        return tool.XML.collect_unique_attrs(cls.root)

    @classmethod
    def get_types(cls):
        return tool.XML.enumerator(
            tool.XML.iterator(cls.root, tag="Object"), attr="type"
        )
