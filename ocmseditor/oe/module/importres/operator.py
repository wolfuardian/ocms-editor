...
def op_import_resources(self):
    tool.Log.info(__name__, "Importing resources...")
...
def import_resources(self):
    ...
    task_collect_fbx = TaskCollectFBXs(
        tool.Registry.get_value(c.REG_KEY, c.REG_SUB, c.REG_PROJ_DIR, ""),
        ocms.ResourcesStore.props["models_paths_mapping_by_type"],
    )
    ...
    task_sync_xml_nodes = TaskSyncXMLNodes(ocms.XMLStore.root)
...
def construct(self):
    ...
    self.groupvbox.add_group(
        widget_id="匯入結果", widget=qt.QtGroupVBoxCSWidget(text="匯入結果")
    )
